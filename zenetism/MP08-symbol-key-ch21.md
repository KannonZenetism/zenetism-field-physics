...
| ⊼ | **Apophatic Tetrad** | The four-fold negation — not this, not that, not both, not neither — spoken toward 🕳️ Zenon from within structure, totalizing by emptying rather than by individuating. Taken alone its subtractive movement bottoms out at ⚫ Aion, the Zero in which every determinate assertion has been withdrawn; negation cannot cross beyond Aion merely by continuing to negate. Distinct from the Trace of Unknowing, which names the residue left at that resting rather than the operation itself |
