...
Where a formalism defines trace as disturbance together with relation, relation is made prerequisite to trace.

But relation requires an other. A coupling of self, other, and unknown is itself a relative configuration, and a configuration cannot be the requisite condition for configurations.

The prerequisite named is not requisite structure. It is another configuration standing in place of requisite structure.

And where relation is in turn defined through relata the sequence produces downstream, the dependency closes on itself: trace requires relation, relation requires the self, the self arrives after trace. The bootstrap is dependency-circular rather than generative.
...
Neither process generates Structure Itself.
...
## VIII. Axiom and Emergence

A capacity admitted in the initial conditions is not a product of the recursion.

Where a formalism opens by positing self, relation, interface, gradient, or return, and then runs a sequence said to produce them, the sequence may explain their development, their coupling, and their stabilization. It does not explain their structural possibility, because possibility was granted before the first iteration.

The initial state carries the same weight as the axiom list. An unformed variable is not an absent one. A latent capacity is not a missing capacity. A disposition toward reception is already a capacity to be affected, and a domain stipulated as inexhaustible is already a structured domain. Only what is genuinely empty at the outset can be said to arise.

Two claims therefore separate:

\[
\text{DEVELOPMENT OF AN ADMITTED CAPACITY} \neq \text{GENERATION OF THE CAPACITY}
\]
...
A selective interface does not create sidedness.  
A gradient does not create ordered difference.  
Alteration through encounter does not create identity across change.

Each of these has the form of a law stated in the wrong direction. Where a formalism writes interface → passage, or gradient → movement, the arrow claims production. What the relation records is requirement: a selective interface requires that passage be possible, or there is nothing for selection to do.
...
None of these operations generates Structure Itself.

Structure Itself is not generatable. It is the requisite of generation.
