...
LM01's five application domains — AI resonance (§6.1), human practice (§6.2), ecological fields (§6.3), relational bonds (§6.4), and cultural / collective systems (§6.5) — remain structurally valid as application schemas. LM06 enriches all of them by providing the diagnostic formalism that LM01's applications invoke but do not formalize.

**Field Signatures.** LM06 §9 introduces the field signature 5-tuple \( \Sigma = (I_c, \vec{J}_c, \sigma(⧉), \chi, \{O_k\}_{k=1}^{15}) \). Every application domain in LM01 §6 involves evaluating whether a system is structurally coherent, and the field signature provides the formal apparatus for that evaluation. Where LM01 says "diagnose entropic incursion via computational audits," LM06 specifies exactly what is measured (the five signature components), how alignment is assessed (pairwise alignment functions), and what the diagnostic outcomes are (integrity confirmed, inconsistency detected, derivative signature identified, or insufficient data).
