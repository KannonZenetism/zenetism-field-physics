...
**Alignment \( \theta \) maps to orientation \( \chi \).** LM03 §3 replaces the alignment angle with the continuous orientation parameter \( \chi \in \mathbb{R}_{> 0} \), where \( \chi < 1 \) is centropic, \( \chi = 1 \) is unstable equilibrium, and \( \chi > 1 \) is entropic. The orientation parameter is more informative than a simple alignment angle: it carries dynamics (the \( \chi \)-evolution equation, LM03 §7), spectral consequences (the rotation function \( r(\chi) \), LM03 §4.5), and embodiment corrections (the resistance-corrected \( \chi \)-equation, LM06 §7).
