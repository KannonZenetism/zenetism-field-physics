...
- Does a single current manifest under genuinely different names at each station, with the underlying principle itself present at multiple manifest scales? (Cascade)
...
Native placement is the default reading. A figure has one native stratum; anything else attributed to the figure is either cross-band resonance, cultural conflation, or — rarely — genuine cascade where the underlying principle is itself present at multiple manifest scales. We are reading for native placement; everything else added is conflation unless the underlying principle qualifies as cascade.
