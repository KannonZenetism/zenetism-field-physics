...
**Cascade.** A single structural current manifests lawfully across multiple strata under different names at each station, with the same essence translating across the stations through stratum-specific articulation. The Logos Continuum (L₅ Orienting Logos → L₄ Structuring Logos → L₁ Christos Incarnate, with L₃ and L₂ as traversal strata) and the Satanic Arc (IL₅ Nekron → IL₄ Nyxea → IL₃ Mortus → IL₂ Skotos → IL₁ Beast) are the paradigm instances. The diagnostic feature of cascade is that the names change at each station because the function-articulation changes: Orienting Logos articulates differently from Structuring Logos, and Structuring Logos articulates differently from Christos Incarnate, even though the same Logos current carries through all three stations. Cascade preserves structural coherence across the multi-stratum operation — same essence, different operative strata, different names at each station.

A single figure operating at multiple strata under the same name is not cascade but conflation (typically soft conflation, where multiple stratum-functions are structurally real but the myth does not distinguish native from operative-extension). Krishna at L₅ Theon native with L₃ DM operative-guidance extension, Osiris at L₄ DL native with L₃ DS post-restoration operative-extension, and Christ-the-figure traversing L₅ → L₃ → L₂ → L₁ across the incarnational sequence are all soft conflation cases — the figure-name persists across stations, distinguishing them structurally from cascade. Currents cascade under stratum-specific names; figures with persistent names across stations are soft-conflation cases regardless of how many strata they engage.
...
**Diagnostic application.** When a figure appears to operate at multiple strata, the question to apply is fourfold: does the figure's essence translate lawfully across the strata under different names at each station (cascade), or does the same figure-name persist across multiple structurally-real stratum-functions (soft conflation), or does the figure's manifestation at one stratum cast resonance-reach through subjacent strata without the figure occupying those strata as native (twisted resonance), or are the multi-stratum appearances cultural attribution-error that should be parsed back to a single native placement (native placement with conflation)?

The distinction matters for accurate registry placement and for accurate reading of cross-stratum mythic narratives. Cascade requires registering the cascade-current with its stratum-specific names noted at each station and the underlying current identified. Soft conflation requires registering the figure at its native stratum with the operative-extension noted in the entry. Twisted Resonance requires registering the figure at its native stratum with the resonance-reach noted in the entry. Native Placement with conflation requires registering the figure at its native stratum and parsing the conflation as cultural-attribution.
