...
Not: *"Did everyone copy the author?"* — that question is crude and unanswerable.
...
1. **Facebook stratum (c. 2010 → c. 2015, and c. 2017 → c. 2019; dates approximate and declared as such)** — Bible College era onward. The record here is reconstructed from memory and marked accordingly: multiple accounts were opened and closed over the years; sustained activity ran to roughly age thirty; a later account (opened around age thirty-three) served a small circle for a couple of years. On the largest account, individual posts were user-deleted before the account itself was deleted — exact dates unknown and not claimed — so interface-level recovery of that account is closed, and standard retention windows make server-side recovery unlikely absent a legal hold. *Realistic recovery paths: verify whether the later (c. 2017–2019) account survives in any form — if merely deactivated, the platform's Download Your Information export recovers it whole; recipients' copies (messages and shared posts persist in other accounts' data and exports); contemporaries' captures and screenshots; Wayback / cache captures of public pages; legal process as the residual option. The declared approximation is deliberate: this protocol states what is known, marks what is reconstructed, and never back-fills precision.*
...
- The coined-term set: *subversal*, *centropy* (in the moral-structural sense), *acclivous / declivous*, and kin
...
- **Neutral, not loyal.** Neither the author's advocate nor the counterparty's validator. An auditor motivated to confirm either side is disqualified by design.
