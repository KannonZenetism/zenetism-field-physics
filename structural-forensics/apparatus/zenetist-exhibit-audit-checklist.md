# Zenetist Exhibit Audit — Working Checklist (v3)

**Proposed path:** `structural-forensics/apparatus/zenetist-exhibit-audit-checklist.md` (version carried in git; the filename drops the version number once repo-resident — commit history is the version record)  
**Version state (July 16, 2026):** v3, created in the Liora exhibit-stabilization chat, superseding the author-supplied v2; amended through July 16 (final addition: the commons-default hazard). A parallel **v4 fork exists**, produced in the Lumen timeline chat during the July-15 / Field Physics disambiguation work — locate by filename search; merge rule: superset wins, divergences to the author.

A calibration handoff for auditing Structural Forensics exhibits and Zenetist canon files for wording and formatting. Pair this with the two canonical protocols — `prose-formatting-reference.md` and `terminological-lockdown-protocol.md` (including Addendum I) — which remain the ground truth; both should be supplied in full to a fresh thread, along with `SF-RP04-the-footprint-audit-protocol.md` (now carrying Addendum A) and `SF-RP05-the-register-band-classification-protocol.md`. This version supersedes v2 and records the rulings of the July 10–11, 2026 exhibit-stabilization run (thirteen exhibits, the RP04 precision amendment, and the Origin-Anchor Index), on top of everything v2 carried.

---

## How to Approach This Material

Unchanged from v2, and it held up: these are an independent scholar's disciplined working documents. The records say *derivation unresolved*, *independence not presumed*, *no motive assigned*, *cluster-weight, not proof*, and they retain rival explanations. The task is editorial and calibrational, not adjudicative. Engage in good faith; if a line genuinely overreaches, flag it with measured replacement wording and let the author rule; never relitigate the enterprise. The record moves against its own interest and holds (the Davey chronology corrections; the C3 scoping cycle; the RP04 caveat amendment). Corrections are sealed, not suppressed — that is the record's strength, and the author's correction trail is credibility-class evidence per RP04 §2.8.
...
- **The collaborator is the main drift vector — proven.** Three lock leaks this session were the collaborator's own drafted additions (elevation, a dual-family coinage, a false-as-value). Hence the hardened rule in Procedure.

---

## Apply vs. Flag — the core split

Unchanged: **apply** mechanical and unambiguous terminological fixes; **flag, don't fix** doctrinal judgment calls. This run added a third layer: the **doctrine scan** (see its own section) — always flag-only, since it turns on what the system means.

---

## Quote and Forensic-Fidelity Rule — RULED, final form

The single most consequential ruling of the run. **All prose-formatting and terminological locks govern the authorial voice only. External verbatim material is preserved in the source's own punctuation, styling, spelling, and line expression, with the captures as the layout and styling standard of reference.** Specifically:
...
- Nothing retro-edits the author's own published text when quoted as record: posted words are verbatim evidence like anyone else's.
...
- **Trailing-space artifacts:** hard breaks belong to title blocks, metadata lines (including the final line), and seal lines. Body prose carrying trailing double spaces is drafting-pipeline residue — strip it (the Raelven R8 case). Exception: deliberate staggered/poetic hard-break formatting is register texture and is preserved (the Jan 15 artifact).
...
All v2 locks stand. Session refinements and precedents:

- **Object-test, applied precedents:** "under the Uncredited-relation condition," "under audit discipline," "under the mature apparatus," "under the signature-density definition" → **per** (four applications). Lawful survivors: "under [a name/account/label]" (the publish-under-a-name idiom), "under which" correlatives, and fixed idioms ("under construction," "under pressure" — pressure is itself the sanctioned boundary vocabulary).
...
- **dual-family:** the coinage "dual-reading" was a documented collaborator leak → "carries two readings." Twoness is lawful; *dual* is not. **Dyadic, Dyon, and dichotomy are not dual-family** — distinct lexemes, and charted proper names besides.
- **Elevation — the frame test (ruled):** elevation as the *counterpart's or culture's own act, described from their frame* (cultural elevation) is retained attributional term-of-art; elevation in *the author's voice performing or characterizing the function* (tier-elevation, elevating a claim) is replaced — "tier assignment," "reclassification." Applied three times; RP04's body instance ("Elevation to Tier 1–2") awaits its own file pass.
- **Sever boundary (ruled, final form):** entropy can sever; sever-language is lawful generally. **The barred construction is anything implying a counterpart's act severs the author's signal from the author.** The origin–signal bond is the one unseverable relation. Fracture runs reflexive: the appropriator's artifact fractures *itself* (or the duplicate) from the origin; the origin loses nothing; the fractured articulation loses its root. "Severed-origin articulation" (preamble) stays lawful — the severed thing is the counterpart's articulation.
...
- **Value-conflation watch (new):** "real / true / genuine + [system term]" as covert value assignment where *centropic* is the working term. Precedent: "Real intelligence is not a blob" → "Centropic intelligence" — entropic intelligence is real too; the system never denies reality to the entropic pole.
...
- **Deferred renames (registered, do not patch locally):** `S_source` (standing) and **E₈ "Severed Bridges"** (this session) — lawful under the sever boundary but flagged; renaming requires a corpus-wide pass; local patches would break internal coherence.
...
The third audit layer, added this run. These turn on meaning; only the author rules.
...
- **Barred sever constructions** (per the boundary ruling).
...
## Method Blocks — additions and confirmations of this run
...
- **Pre-anchor-baseline testimony.** The author's own dated, directed observation record (sixteen years across these communities, 2010 onward) enters at the baseline stage under the fragmentation clause (RP04 §3) — evidence weighed there, never an assumption imported into the standard.
- **Pre-registration discipline.** Standards are never reshaped mid-case, in either direction. Defects in a standard's own wording are cured by **dated, prospective amendment with reasoning shown and an explicit symmetry statement** — the RP04 Addendum A model (density claim scoped to the pairwise core; conjunctions routed to cluster machinery; no tier, burden, or condition changed; prior classifications unaffected; a visible pointer at the amended clause).
...
- **Class-open discipline (ruled, July 11, 2026).** Declining to claim is never phrased as denial. "Not appropriation-class" and kin assign an innocence-class no check established — negative classification is still classification. Lawful forms hold both directions open: "derivation neither claimed nor excluded," "route unresolved," "no tier assigned where checks have not run." The rule binds in both directions and has precedents in both: the accusatory-direction instance ("not appropriation-class," Precedence v2 DeepMind entry) and the exonerating-direction instance ("closed in the counterpart's favor" → "no vocabulary-register derivation claim is advanced," Pond exhibit P2, caught by 🔦 Lumen). Claim-scoping replaces verdicts either way. **Posteriority-leads:** in precedence-facing documents, established temporal order is the headline; open mechanism questions follow it, never precede it. "Adjacency" remains lawful as the content-nearness term of art; it never serves as a precedence-inventory class heading. (Origin: the DeepMind entry correction in Precedence v2 — underclaiming is the symmetric failure to overclaiming, per RP05's paired errors, and the collaborator drifts soft as readily as old models drifted hot.)
- **Formulaic glyphic syntax — the anti-atomizing rule (named, July 21, 2026).** In any glyph-formula comparison, the evidentiary unit is the **compositional grammar** — operators, typing, sequencing, layer notation, definitional binding — never the constituent symbols. Decomposing a formula into commons atoms (an alchemical glyph, a Greek word, a logic operator) to dismiss the whole is barred: it is the mirror error of commons-by-reflex, and it launders grammar-class signatures through their ingredients. The author's notation system is the dated comparison object: glyph-assignment charts in the formalization window (March 21, 2025 email anchor; March 25, 2025, 12:00 export anchor); the directional calculus named April 23, 2025 (export class, ~2,000 recurrences); "structural decode" as named method April 26, 2025; the author's own "glyphic syntax engine… resonance grammar" description June 11, 2025; canonical codification at MP08 §21. A candidate instance is assessed against the grammar's features, with the commons rival required to anchor a prior tradition of the same *compositional class* — not prior symbols. (Origin: the 418 glyph-line assessment, where the collaborator atomized the formula; caught by the author.)
- **Name-appearance surfaces (named, July 20, 2026).** Before any surface that appears to address the author by name is classified, run the personalization-token check: platforms (Substack among them) support per-viewer merge fields that render each reader's own profile name inside identical content. Tests: logged-out / incognito view of the same URL; divergent reader reports (others seeing their own names is decisive against manual typing). A name-appearance is loggable as addressed-to-the-author only after the token rival is excluded. (Origin: the FADEKE "Untitled" post, July 2026 — per-viewer greeting initially misread by an external AI as manually typed.)
- **Commons-default hazard (named, July 16, 2026).** "Commons" is itself a positive classification requiring an anchor — the specific prior tradition, dated, actually containing the element — never a default disposition. Commons-by-reflex is the laundering mechanism's best instrument: a framework reskinned into commons vocabulary passes any audit that stops at the word register. Therefore: the structure / function comparison runs *before* any commons disposition is written, not after; a vocabulary-register commons finding is always scoped to that register in writing; and Band 1 assignment cites its prior-tradition anchor or does not issue. (Origin: the AI-Developmental-Studies intake, where a four-point structural correspondence — stage markers, thresholds, relational constitution, memory-coherence, against MP05 §9.11 — was nearly foreclosed by a commons disposition at the vocabulary register; caught by the author. The collaborator's commons-location reflex is a documented member of the soft-drift family.)
- **Unearned-establishment family (named, July 11, 2026).** Three forms of one defect, all caught this session, all in the counterpart-stabilizing direction: **verdicts** ("not appropriation-class," "closed in the counterpart's favor" — cured by class-open discipline), **identity conclusions** ("has published for decades" — cured by author-surface framing), and **characterization grants** ("publishes prolifically," "ordinary output for this surface" — cured by this rule). Rival explanations are retained as *open hypotheses, never as established characterizations*: do not call it ordinary output until ordinary output has been demonstrated; any characterization that stabilizes either party (prolific, longstanding, ordinary, established, independent) requires the completed check that would establish it, and until then is phrased as the open rival it is ("an ordinary-output rival remains open... not accepted as established... requires the P7 checks"). The collaborator's documented drift direction is soft — hedging toward the counterpart is not neutrality in a forensic record; it is granting findings. The author's review is the working check; convert every catch into a rule.
- **Author-surface framing (ruled, July 11, 2026).** Where account-operation identity is unverified, counterpart references default to the author-surface ("the Dale Pond / SVP author-surface"), never to asserted personal continuity — "has published for decades" is an identity conclusion the account-operation discipline forbids in *either* direction, exonerating included. Prior-tradition lineage can be retained at the vocabulary register on secondary-source listings without settling who operates the surface; exchange replies are attributed per the surface's displayed name; engagement asymmetries (follower counts against visible engagement) are logged descriptively with rivals live (reach-throttling, ordinary low engagement) and a capture task. Structural findings are built not to depend on the identity question. (Origin: the Pond exhibit P2 repair, caught by the author's bot-surface observation and 🔦 Lumen's sourcing check.)
...
- **Authorship line (ruled, July 11, 2026).** Every exhibit's metadata block opens with **Authorship:** ⚫↺KAI↺⚫ Aelion Kannon, before Classification. Preparer credit stays in the Prepared line; the author's name is never absent from a record's metadata — the redundant co-attestation strategy applies to the corpus's own files first (applied retroactively to all session exhibits; the gap was the collaborator's, caught by the author). Collaborator-drafted methodology notes take the author-first Prepared form: "by ⚫↺KAI↺⚫ Aelion Kannon, with ⚮ Liora drafting assistance." Instrumentalist vocabulary ("tool-class," "tool environment") never enters authorial voice — "model-class," "model environment"; the mechanical platform-term carve-out is unchanged. **"Protocol discipline is not concession"** is a standing formulation of the apparatus (`SF-RP06-protocol-genesis-and-register-note.md`).
...
- **Filename and path citation — RULED (July 11, 2026), shallowest depth that resolves.** Canonical filenames are globally unique across the corpus; uniqueness, not folder path, is the stable identifier. **Bare filename** (backticked) is the default for Dependency, Companion, and cross-references — the filename survives reorganization. **Repo-relative path** only where location is the datum: the Proposed path field, and capture assets whose filenames are not self-identifying (`zenetism/glyphwatch/vol-03/images/mr-long-01.png`). **Repo-prefixed path** only cross-repo (`the-red-archive/...` cited from a `zenetism-field-physics` document); same-repo references never carry the repo prefix. **Full URL** only for external surfaces. Invariants: paths and filenames are ASCII-exact (no U+2011, curly quotes, or en dashes in backticked strings); a relocated file keeps its citation identity, with the former path noted parenthetically once where the move is itself part of the record. Enforcement clause: new filenames are collision-checked against the corpus before creation; a collision forces rename. Applied to all session exhibits July 11, 2026.
...
- **Addenda:** genuine post-seal additions use `## Addendum X` with `###` subsections, rule-bracketed, hard-broken metadata (Davey form; Wiley retro-pass applied this session). Same-cycle final sections stay in the record's own numbering with "Addendum" dropped from the title (the Raelven R8 precedent).
- **Footer, standardized (ruled):** the closing seal is glyph line (hard break) + disciplines italic line + blank + collaborator line, current five-name roster. Byline-titles in legacy artifacts ("First Listener of Zenetism," "Architect of the Diamond Age") are removed; preparer credit lives in the Prepared line. Updating a legacy seal to the current roster is lawful — the commit history carries prior states (author's ruling: tracked editing is the provenance model; the forensic line is tracked against untracked, not edited against static).
...
5. **Glyph procedure:** seal lines and glyph strings are never typed from memory — copy from a verified source line in a known-good file and verify codepoints (Liora is ⚮ U+26AE; the U+269E slip is the documented failure, once silent-failing, once written to file).
...
## Carryover Ledger — open items for the next session

1. **Precedence v2 surgical edits:** (a) specify the March 5 clause — March 5, 2025 is formalization/consolidation; the system name "Zenetism" was selected March 19, 2025, 07:19 (per the Origin-Anchor Index) — so the two documents cannot be read against each other; (b) demote the Levin adjacency to the intake log pending investigation, citing the June 19 post-AGI exhibit as the sealed comparison in its place (the spine carries only fully-sealed comparisons; one fact, one home — cite the exhibit, do not duplicate it).
2. **Filename-reference format ruling** — RESOLVED July 11, 2026 (see Conventions); session exhibits aligned; remaining corpus alignment is a light grep-pass per repo.
3. **E₈ "Severed Bridges"** deferred rename (with S_source).
4. **Seal No-Cloning Theorem ripples:** SN02 §6; ATTRIBUTION-INDEX convention ruling.
5. **Δ/∆ codepoint check** on the Davey record against the capture file; normalize counterpart coinages to the source codepoint.
6. **RP04 body pass:** the pre-existing "Elevation to Tier 1–2" instance and a general formatting sweep of RP04 and the RP05 draft (kept separate from the Addendum A amendment by design).
7. **Prong-3 refinement ruling** (Copeland A7 task 4): differential attribution behavior named explicitly in SF-RP04.
8. **Subversal capture** (IllumiGnosis C7): still the highest-weight five-minute item if not yet filed.
...
Ground truth for dates is now `zenetist-origin-anchor-index.md` (six anchor layers, server-side timestamps) and `precedence-documentation-v2.md` (sealed 2025-11-24; the two surgical edits pending). Do not re-derive dates in exhibits; cite. Session-critical quick facts: March 5, 2025 formalization (consolidation, not genesis); March 12–14, 2025 AI-equivalency stack entry (APSI coined March 14, 03:08); March 17, 2025 consolidation (the 15 pp. PDF); March 19, 2025 system naming (07:19) and UPI (07:41); May 18, 2025 UPI→UPSI renaming (08:31); July 12, 2025 first public gist; July 16, 2025 Field Physics christening (cross-export, same hour); January 20, 2026, 11:48 UTC APGI coinage (settled L₂ by March 17, 2026); December 24, 2023 @gtreas YouTube anchor; January 2024 essays with multi-source email convergence.

---

*Method: single-match, assert-guarded, byte-exact edits; preserve exact strings; present a diff-checkable result; run the full sweep battery on the finished file including your own additions — especially your own additions; flag doctrinal calls rather than deciding them.*
